...
class DeferredResolver extends sangria.execution.deferred.DeferredResolver[BGraphQlTypes.MyCtx] {
  override def resolve(deferred: Vector[Deferred[Any]], context: BGraphQlTypes.MyCtx, queryState: Any)(implicit ec: ExecutionContext): Vector[Future[Any]] = {
    deferred.map {
