...
import uk.gov.nationalarchives.tdr.api.core.graphql.{BGraphQlTypes, DeferredResolver, GraphQlTypes, RequestContext}
  ...
  def send(request: GraphQlRequest): Future[Json] = {
    ...
    query match {
      case Success(doc) =>
        ...
        Executor.execute(
          BGraphQlTypes.schema,
          doc, BGraphQlTypes.context,
